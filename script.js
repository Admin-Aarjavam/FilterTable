        ...
        const observer = new MutationObserver(mutations => {
            mutations.forEach(mutation => {
                mutation.addedNodes.forEach(node => {
                    ...
                    // If full table is added
                    if (node.nodeType === 1 && node.classList?.contains("data-table")) {
                        sg_initTableFilters(node);
                        sg_makeCellsFocusable(node);
                    }

                    // If content added inside table (rows, td, etc.)
                    if (node.nodeType === 1) {
                        const tables = node.querySelectorAll?.(".data-table");
                        tables?.forEach(t => {
                            sg_initTableFilters(t);
                            sg_makeCellsFocusable(t);
                        });

                        const row = node.closest?.("tr");
                        const table = node.closest?.("table");

                        if (row && table && table.classList.contains("data-table")) {
                            sg_makeCellsFocusable(table);
                        }
                    }
